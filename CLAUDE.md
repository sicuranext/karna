...
- **`ka_redact.lua`** — Audit-log secret redaction. Compiles a spec from the config (`auditlog_redact_enabled` default true, `auditlog_redact_headers`, `auditlog_redact_mask`) and masks header VALUES in the built audit document: v2 `request.headers` / `response.headers`, v1 the same under `transaction`. Per-name shapes: `authorization` / `proxy-authorization` keep the scheme (`Bearer [REDACTED]`, masked whole when the value has no space), `cookie` keeps the cookie names and masks values, `set-cookie` also keeps the attributes (Path / HttpOnly / SameSite / Expires) and always masks the first pair. Scope is the two header maps and NOTHING else — matched values, the URI, `body_raw`, custom log fields and enrichment are untouched by design (a rule that catches a secret by accident is the CRS-tuning signal). Called from `handler.lua:log` as the last step before the async writer; the spec is cached via `get_redact_spec_cached` (conf_cache_key + `false` sentinel for "nothing to mask", since an LRU cannot hold nil). Absorbed the old `ka_mcp.redact_audit` recursive walk: the two `mcp_redact_*` toggles are now inputs to the spec, session id keeps its `first4 + ***` shape. Pure Lua, no kong/ngx globals, so `ka-unittest/auditlog_redact.lua` requires it directly.
...
- **Rule controls**: Mechanism to remove rules, remove specific targets from rules, or modify rule conditions at runtime (similar to ModSecurity `ctl:ruleRemoveById`). The per-request family (`ctl:*` surface, applied by `ka_engine.lua:__apply_rule_controls_inline` into `kong.ctx.plugin.rule_controls`, nothing persists): `remove_rule`, `remove_rules_by_tag`, `remove_target_from_rule_by_id`, `remove_target_rule_by_tag`, `engine_off`, `detection_only`, `engine_on` (`ctl:ruleEngine=On` — forces terminal actions for the rest of the request even on a detection-only service or after `detection_only`; `engine_off` wins, DetectionOnly/On last-applied wins; records `engine_forced_by`), `body_access_off`, `audit_request_body` (`ctl:auditLogParts=+C` — attach the raw body to the audit record, pure enrichment, never forces a write). Controls on a terminal rule are applied BEFORE its own action is dispatched (`handler.lua:evaluate_rules`, ModSecurity order), on every path (global pack detection list, `rules_request`, `custom_secrules`). SecLang reads `ctl:*` from every chain link (the bundled CRS has none on children). Same two channels always: JSON `rule_control` and `seclang.lua:__get_rule_controls`.
  - **Target removal shapes** (`ka_engine.lua:remove_ctl_target`, one applier for by-id / all-rules / by-tag / load-time names): per-name `request.cookie.value:<n>` strips the plain key AND every derived key (`request.cookie.json.<n>.*`, `request.query.json:<n>.*`, `request.body.urlencode.json:<n>.*`, `…:<n>_ka_b64_decoded`, `…:<n>:2` duplicates; names compared lowercase); collection form = bare namespace (`request.cookie.value` = `REQUEST_COOKIES`, `request.cookie.name`, `request.header.value`, `request.arg.value`, `request.query.value`, …) empties the map when the rule resolves that collection, or only the folded part when it resolves a superset (`FOLDED_COLLECTIONS`: ARGS ⊇ query/body namespaces + ARGS_NAMES; cookie value map ⊇ cookie names). Namespace gate always holds. The matcher copies the cached values map lazily before the first deletion (`values_private`) so no removal reaches another rule through `ka_variable_cache` or the getters' own caches.
- **Rule actions**:
  ...
  - `rate_limit` — Redis-backed fixed-window rate limiter. Fields: `key` (macro string, default `%{remote_addr}`), `limit`, `window_seconds`, optional `response`, and `log_all_matches` (default false). Increments the counter `karna:rl:<rule_id>:<resolved_key>` and returns 429 (with auto `Retry-After`) when the counter exceeds `limit`. Counter is bumped regardless of `engine_blocking_mode`; the terminal 429 only fires in blocking mode. With rule-level `log=true`, enforcement events are audit-eligible by default; `log_all_matches=true` also records admitted matches. Macro resolution in access phase uses a lightweight inline resolver (`resolve_request_macros` in handler.lua) — supports `%{remote_addr}`, `%{request.method|host|scheme|path}`. Unsupported macros stay literal (fail-soft).
...
- **Variables**: Internal naming convention like `request.arg.value`, `request.header.value:host`, `request.cookie.name`, `request.body`, etc. Note: `request.query.value:<name>` IS resolvable in `__match_rule_conditions` (own branch, query string only); `request.arg.value:<name>` stays the canonical choice when you mean any arg (query + body urlencoded). **Two variable surfaces, and they are not the same set**: conditions resolve only the names with a branch in `__match_rule_conditions_impl`; `geoip.*`, `asn.*`, `var:*`, `request.header.name:*`, `request.header_no_fp.value:*`, `request.header.referer.*` and `request.forwarded_scheme|host|port|path|prefix` exist ONLY in the inspection table (built in `header_filter`), so they work as `%{...}` macros and resolve to nothing in a condition — the rule then never fires, silently. `var:paranoia_level` resolves nowhere: the PL gate is `rule.paranoia_level` in `loop_rules`, and the seclang line that injected that condition is commented out. Documented in `docs/rules.html#macro-only-vars` and enforced by `docs/schema/karna-rule.schema.json`.
  - **Request getters are self-less** (`__get_values_request_cookie(try_b64)`, `__get_values_request_query_value`, `__get_values_request_body`, `__get_values_request_headers_all`): call them with a DOT, never a colon (a colon passes the engine table as the first argument; for the cookie resolver that turned `try_b64` truthy and switched the base64 pass on for every JSON cookie, regardless of `try_bas64decode_if_possible`). `__get_values_request_args(self, try_b64, rule_control)` is the exception (colon). The rule-matching path passes a literal `false` for `try_b64` on every collection (ARGS / query / body / cookies): the schema flag reaches only the gates (`check_request_body_parser`, `check_request_arg_count`), `get_inspection_table` and `uri_path_check_violation`, not the request-phase resolvers. The cookie resolver also normalises `try_b64` to a strict boolean. Unit-tested in `ka-unittest/cookie_b64_resolver.lua`; engine-loading unit tests share `ka-unittest/_engine_harness.lua`.
...
- **Cookie parsing is total**: `__get_values_request_cookie` splits on `;` with a helper that iterates `(input .. ";")`, so an empty segment (trailing `;`, leading `;`, doubled `;;`) is normal input, not an edge case. A segment with no name is SKIPPED — it used to crash the resolver on nil (`string_gsub`) and 500 the request, or insert a junk `request.cookie.name:` / `request.cookie.value:` key under an empty selector. The one empty-name shape that still carries attacker bytes, `<whitespace>=<value>`, is kept inspectable as `request.cookie.value:__ka_unnamed_cookie_<n>` (value only, no synthetic NAME, added whole and not re-parsed) — same idea as `__ka_path_confusion_<n>` above, so skipping empty names is not an inspection gap. The per-segment pattern `([^=]+)=?(.*)` is unchanged, so a key-only cookie (`Cookie: foo`) stays a name-only cookie.
...
- Raw request body: only when the `audit_request_body` rule control fired on the request (and `body_access_off` did not). v2 `request.body_raw` + `body_encoding` (`utf-8`|`base64`) / `body_truncated` / `body_length`; v1 `transaction.request.body` (ModSecurity part C slot) + same siblings. Clipped at `auditlog_request_body_max_bytes` (default 16384, schema). The bytes come from `kong.ctx.plugin.ka_raw_body`, pinned by the engine's single cached reader `get_raw_request_body()` in the request phase (`kong.request.get_raw_body` is not callable in `log`); built by `ka_utils.lua:build_audit_request_body` / `attach_audit_request_body` (unit-tested in `ka-unittest/audit_request_body_control.lua`). It logs whatever the client sent (credentials on a login form) — documented as such.
- `engine.mode` (v2) is the EFFECTIVE mode for the request: `blocking` when `engine_blocking_mode` and no `detection_only`, or when a rule forced `engine_on` (then `engine.forced_by_rule` names it). Per-match `action` is decided per entry from `match_entry.blocked` (set by the handler at dispatch), so an earlier detect stays `detect` when a later rule forces On and blocks. v1: tag `karna/engine-forced-on/<id>` on the message. Unit-tested in `ka-unittest/engine_on_control.lua`.
- Secret redaction runs in the log phase, after the document is built and after `attach_audit_request_body` / `additional_log_fields`, so it also covers what those add to the header maps. See `ka_redact.lua` above for the scope decision.
...
- `kong.ctx.shared.karna.log_entries` (array) — sibling plugins can append `{source, rule_id, message, tags?, metadata?}` records here and Karna will emit them as `external_matches[]` in the audit log v2. Malformed entries are silently dropped; oversize strings are clipped (`source`/`rule_id` at 100B, `message` at 1000B). The presence of valid entries forces an audit log write even when no Karna rule matched (overrides `auditlog_only_on_match`). Under `auditlog_format = "v1"` the same entries become extra `transaction.messages[]` elements (`message` + `details.ruleId` / `details.data` = `"Source: <source>"` / `details.tags`) — no new keys in the v1 document, and `metadata` is deliberately dropped there (no structured slot; free-text flattening is a secret-leak path). Normalisation lives in `ka_utils.lua:build_external_matches`, the v1 mapping in `ka_utils.lua:build_v1_external_messages` (unit-tested in `ka-unittest/external_log_entries.lua` and `ka-unittest/external_log_entries_v1.lua`).
