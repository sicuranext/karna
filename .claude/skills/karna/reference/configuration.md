...
## Audit logging
- `auditlog_enabled` (bool, `true`), `auditlog_path` (str, `/usr/local/openresty/nginx/logs`, must be kong:kong-writable), `auditlog_format` (`v2`|`v1`, default `v2`), `auditlog_only_on_match` (bool, `false`), `auditlog_modsec` (bool, `false`), `auditlog_error_log_on_match` (bool, `false`), `auditlog_request_body_max_bytes` (num, `16384` — cap for the raw body attached by the `audit_request_body` rule control; clipped above it with `body_truncated: true`).
- **Secret redaction** (on by default, both formats): `auditlog_redact_enabled` (bool, `true`), `auditlog_redact_headers` (array, default `authorization, proxy-authorization, cookie, set-cookie, x-api-key, api-key, apikey, x-auth-token, x-access-token, x-session-token, x-csrf-token, x-xsrf-token, x-amz-security-token` — the list REPLACES the default, `[]` disables), `auditlog_redact_mask` (str, `[REDACTED]`). Masks header VALUES in `request.headers` / `response.headers` (v1: under `transaction`). Shapes: `authorization` keeps the scheme (`Bearer [REDACTED]`; no space in the value → masked whole), `cookie` keeps the cookie names and masks values, `set-cookie` also keeps the attributes (Path/HttpOnly/SameSite/Expires) and always masks the cookie itself. NOT redacted by design: matched values (`matches[].matched_parts[]`, v1 `details.data` — the rule-tuning signal), the URI/query string, `body_raw`, custom log fields, enrichment. Log phase only, spec compiled+cached per plugin config, one lookup per configured name per written record. MCP toggles `mcp_redact_authorization_in_audit` / `mcp_redact_session_id_in_audit` feed the same spec (session id → first 4 chars + `***`), only when `mcp_enabled`.
...
- Every audit entry also carries `network.connection_id` (v1: `transaction.network.connection_id`, pseudonymous `kc1_<32 hex>`, stable per TCP connection, HMAC keyed by env `KARNA_CONNECTION_ID_HMAC_KEY` >= 16 B, random per-worker key + warning when unset) and a `tls` block (v1: `transaction.tls`): `enabled`, `capture_status`, `protocol`, `cipher`, `curve`, `alpn`, `sni`, `session_reused`, `early_data`, `client_ciphers`, `client_curves` (client-offered colon lists, capped 4096 B). Plain HTTP → `{enabled:false, capture_status:"not_tls"}`. `client_curves` empty on resumed TLS 1.2; `early_data` ~always false (Kong has no `ssl_early_data on`). On `complete` captures also `tls.fingerprint = {version:"karna-tls-v1", algorithm:"sha256", value:<64 hex>}` (spec in rules.md / docs). No secrets/session ids ever.
